...
def trade(data):
    ...
    data.ix[(data['MA_5'].shift(1)<data['MA_10'].shift(1))&(data['MA_5']>data['MA_10'])&(data['VOLUME']>0),'POSITION']=0.3
    data.ix[(data['MA_5'].shift(1)>data['MA_10'].shift(1))&(data['MA_5']<data['MA_10'])&(data['VOLUME']>0),'POSITION']=-0.3
    data.ix[(data['MA_10'].shift(1)<data['MA_15'].shift(1))&(data['MA_10']>data['MA_15'])&(data['VOLUME']>0),'POSITION']=1
    data.ix[(data['MA_10'].shift(1)>data['MA_15'].shift(1))&(data['MA_10']<data['MA_15'])&(data['VOLUME']>0),'POSITION']=-1
    ...
    if len(data[(data['POSITION'] == 1)])==0:
        ...
        data.ix[(data['DATES']>=first)&(data['DATES']<startposition),'POSITION']=0
        data=data.fillna(0)
        ...
        for i in range(0,len(position)-1):
            try:
                if((position[i+1] + Trueposition[i])>=0 and (position[i+1] + Trueposition[i])<=1):
        ...
        for j in range(0,len(DATA)):
            ...
            if(j==0):
                ...
                DATA.ix[j, 'CAPITAL']=capital-DATA.ix[j,'CLOSE']*DATA.ix[j,'STOCKNUMBER']
            else:
                # 实际持仓为0时，卖出持仓的数量和流动资金
                if(DATA.ix[j-1,'TRUEPOSITION']==0):
                    DATA.ix[j,'STOCKNUMBER']=DATA.ix[j-1, 'CAPITAL']// DATA.ix[j,'CLOSE']//100*100*DATA.ix[j,'TYPE']
                    DATA.ix[j, 'CAPITAL']=DATA.ix[j-1, 'CAPITAL']-DATA.ix[j,'CLOSE']*DATA.ix[j,'STOCKNUMBER']
                # 实际持仓不为0时，卖出持仓的数量和流动资金
                else:
                    DATA.ix[j, 'STOCKNUMBER'] =DATA.ix[j-1, 'STOCKNUMBER']/DATA.ix[j-1, 'TRUEPOSITION']*DATA.ix[j, 'TRUEPOSITION']
                    DATA.ix[j, 'CAPITAL'] = (DATA.ix[j-1, 'STOCKNUMBER']-DATA.ix[j, 'STOCKNUMBER'])*DATA.ix[j,'CLOSE']+DATA.ix[j-1, 'CAPITAL']
            data.ix[data['DATES'] == DATA.ix[j,'DATES'],'STOCKNUMBER']=DATA.ix[j,'STOCKNUMBER']
            data.ix[data['DATES'] == DATA.ix[j, 'DATES'], 'CAPITAL'] = DATA.ix[j, 'CAPITAL']
...
def ma(MA,Tradedate):
    ...
    for i in range(len(MA)):
        for j in range(len(Tradedate)):
            try:
                ma = c.css(code, "MA", "Period=1,ispandas=1,AdjustFlag=2,n=" + str(MA[i]) + ",Tradedate=" + Tradedate[j])
                # time.sleep(1)
            except:
...
for i in range(0,len(code)):
    print i
    data=pd.concat([stockdata1.ix[code[i]],stockdata.ix[code[i]],Mx.ix[code[i],[1,2,3]]],axis=1)
...
#上证50指数
indexdata=c.csd(index,"CLOSE",startdate,enddate,"rowindex=2,ispandas=1,AdjustFlag=2")
# 计算上证指数收益率变化值
