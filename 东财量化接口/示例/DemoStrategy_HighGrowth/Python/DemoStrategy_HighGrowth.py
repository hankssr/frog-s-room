...
def pick(number,startyear,code):
    ...
    for i in range(0,number):
        ...
        data=c.css(code.Codes,'EPSBASIC,MBREVENUE','ispandas=1,ReportDate='+reportdate)
        data.rename(columns=lambda x: x + '_' +reportdate, inplace=True)
        EpsMr=pd.concat([EpsMr,data[['EPSBASIC'+'_' +reportdate,'MBREVENUE'+'_' +reportdate]]], axis=1)
        if i > 2:
            # 计算3年EPS增长率，以及3年销售收入增长率
            EPS3=((EpsMr[EpsMr.columns[i * 2]] - EpsMr[EpsMr.columns[i * 2 - 2]]) / EpsMr[EpsMr.columns[i * 2 - 2]] + (EpsMr[EpsMr.columns[i * 2 - 2]] - EpsMr[EpsMr.columns[i * 2 - 4]]) / EpsMr[EpsMr.columns[i * 2 - 4]] + (EpsMr[EpsMr.columns[i * 2 - 4]] - EpsMr[EpsMr.columns[i * 2 - 6]]) / EpsMr[EpsMr.columns[i * 2 - 6]]) / 3
            MBREVENUE3=((EpsMr[EpsMr.columns[i * 2+1]] - EpsMr[EpsMr.columns[i * 2-1]]) / EpsMr[EpsMr.columns[i * 2-1]] + (EpsMr[EpsMr.columns[i * 2-1]] - EpsMr[EpsMr.columns[i * 2-3]]) / EpsMr[EpsMr.columns[i * 2-3]] + (EpsMr[EpsMr.columns[i * 2-3]] - EpsMr[EpsMr.columns[i * 2-5]]) / EpsMr[EpsMr.columns[i * 2-5]]) / 3
            ...
            df=c.css(code.Codes,'YOYNI,YOYGR,YOYEQUITY','ispandas=1,ReportDate='+reportdate)
            FinancialData=pd.concat([df[['YOYNI','YOYGR','YOYEQUITY']],EPS3,MBREVENUE3], axis=1)
            FinancialData =FinancialData.dropna(axis=0, how='any')
            # 筛选3年EPS增长率大于股票池中间值，筛选3年销售收入增长率大于股票池中间值的股票
            FinancialData=FinancialData.ix[(FinancialData['EPS3']>FinancialData['EPS3'].median())&(FinancialData['MBREVENUE3']>FinancialData['MBREVENUE3'].median())]
            # 筛选所有增长率为正的股票
...
def transfer(number,startyear,code):
    ...
    for i in range(startyear,endyear):
            try:
              ...
              data=c.css(','.join(tcode[i]),'NAME,CLOSE,MV','ispandas=1,adjustflag=2,TradeDate='+adjustlist[i-startyear])
              data.insert(0, 'ADJUSTDATE', adjustlist[i - startyear])
            except:
              # 当持仓为空时，判断首次持仓不为空，且前次持仓不为空的，将前次持仓全部卖出
              if(tcode[i] == [] and k == 1):
                  if(tcode[i-1] != [] and k == 1):
                      ADJUSTdata=STOCKdata[STOCKdata['ADJUSTDATE']==adjustlist[i-startyear-1]]
                      ADJUSTdata = pd.DataFrame(ADJUSTdata[['STOCKNUMBER', 'ADJUSTDATE', 'CLOSE']])
                      ADJUSTdata.rename(columns={'STOCKNUMBER': 'NUMADJ', 'CLOSE': 'PRICE'}, inplace=True)
            ...
            # 当持仓不为空时，判断为非首次持仓时，根据前后持仓明细计算持仓变动
                ...
                # 获取调仓当日，调仓前的股票池的收盘价
                data1 = c.css(','.join(tcode[i-1]), 'CLOSE', 'ispandas=1,adjustflag=2,TradeDate=' + adjustlist[i - startyear])
                data['STOCKPRO'] = data['MV'] / data['MV'].sum()
                data['STOCKNUMBER'] = ((STOCKdata[STOCKdata['ADJUSTDATE']==adjustlist[i-startyear-1]]['CAPITAL'].sum()/len(tcode[i]) + (STOCKdata[STOCKdata['ADJUSTDATE']==adjustlist[i-startyear-1]]['STOCKNUMBER']*data1['CLOSE'])).sum())*data['STOCKPRO'] // data['CLOSE']
                data['STOCKAMOUNT'] = data['STOCKNUMBER'] * data['CLOSE']
                #流动资金=调仓前股票当日持股数量*当日价格+调仓前流动资金-调仓后股票当日持仓金额
                data['CAPITAL'] =(STOCKdata[STOCKdata['ADJUSTDATE']==adjustlist[i-startyear-1]]['CAPITAL'].sum()/len(tcode[i]) + (STOCKdata[STOCKdata['ADJUSTDATE']==adjustlist[i-startyear-1]]['STOCKNUMBER']*data1['CLOSE'])).sum()-(data['STOCKAMOUNT'].sum())
                ADJUSTdata=pd.DataFrame(0,index=list(set(tcode[i-1]).union(set(tcode[i]))),columns=["NUMADJ"])
                # 当股票调仓前和调仓后都有时，持仓变动=调仓前持股数——调仓后持股数
                ADJUSTdata.ix[[l for l in tcode[i] if l  in tcode[i-1]],"NUMADJ"]=data['STOCKNUMBER']-STOCKdata[STOCKdata['ADJUSTDATE']==adjustlist[i-startyear-1]]['STOCKNUMBER']
                # 当股票调仓后新买入时，持仓变动=调仓后持股数
                ADJUSTdata.ix[[l for l in tcode[i] if l not in tcode[i-1]],"NUMADJ"]=data.ix[[l for l in tcode[i] if l not in tcode[i-1]],'STOCKNUMBER']
                # 当股票调仓后全部卖出时，持仓变动=-调仓前持股数
                ADJUSTdata.ix[[l for l in tcode[i-1] if l not in tcode[i]], "NUMADJ"] =(-1)*STOCKdata[STOCKdata['ADJUSTDATE']==adjustlist[i-startyear-1]].ix[[l for l in tcode[i-1] if l not in tcode[i]], 'STOCKNUMBER']
                ADJUSTdata['ADJUSTDATE']=data['ADJUSTDATE']
                ADJUSTdata.ix[[l for l in tcode[i - 1] if l not in tcode[i]], 'ADJUSTDATE']=adjustlist[i - startyear]
                ADJUSTdata['PRICE'] = data['CLOSE']
                ADJUSTdata.ix[[l for l in tcode[i - 1] if l not in tcode[i]], "PRICE"] = data1.ix[[l for l in tcode[i - 1] if l not in tcode[i]], "CLOSE"]
                ADJUSTdata['ADJUSTNUMBER'] = i - startyear + 1
                #根据持仓变动的正负判定买卖类型
...
for i in range(startyear,endyear):
    try:
        # 获取每年持仓股票的每周的收盘价
        data=c.csd(','.join(tcode[i]),"CLOSE",str(i)+"/04/01",str(i+1)+"/03/31","ispandas=1,period=2,adjustflag=2")
    except:
        continue
    ...
    # 填充持仓股票每周持股数量
    for n in tcode[i]:
        data.ix[data['CODES']==n,'STOCKNUMBER']=STOCK.ix[STOCK['CODES']==n,'STOCKNUMBER'][(STOCK.ix[STOCK['CODES']==n].index.tolist())[0]]
    data['STOCKAMOUNT']=data['STOCKNUMBER']*data['CLOSE']
...
# 获取上证指数每周的收盘价
indexdata=c.csd(index,"CLOSE",str(startyear)+"/04/01",str(endyear)+"/03/31","ispandas=1,period=2,Rowindex=2")
