...
# 复制指定页面:page_num母版编号,pageNum复印页数量
def Copy_ppt(page_num, pageNum):  # 复制指定页面:page_num母版编号,pageNum复印页数量
    for i in range(pageNum):
        ppt.slides.add_slide(ppt.slide_layouts[page_num])#按照ppt母板的第一个样式加入新的ppt
#删除页面
def Del_page():
...
#构建实例,打开pptx文件
ppt = Presentation(os.getcwd() + "\\" + "笔记本 - 副本.pptx")

# 复制模板页,第4页是月计划(编号4)，5-12月，复印7页
# 注意:通过win32com调用复制,第一页索引号是1
Copy_ppt(1, 8)  # 月计划表:8页
pageNums = int(倒计时() + 1)
Copy_ppt(2, pageNums)  # 复制第7页日计划页,245页

Copy_ppt(3, 8)  # 习惯打卡8页

Copy_ppt(4, 40)  # 每周菜单5*8(5-12月)=40页

Copy_ppt(5, 30)  # 股票记录30页
Copy_ppt(6, 30)  # 康奈尔笔记30页
Copy_ppt(7, 5)  # 方格纸:5页
Copy_ppt(8, 5)  # 米字格:5页
Copy_ppt(9, 5)  # 横线纸:5页
Copy_ppt(10, 5)  # 摘录页:5页
